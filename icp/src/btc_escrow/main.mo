...
import Bitcoin "mo:bitcoin/Bitcoin";
...
stable var addressBook : Trie.Trie<Text, Text> = Trie.empty();
stable var payments : Trie.Trie<Text, { txid : Text; confirmed : Bool }> = Trie.empty();
stable var network : Bitcoin.Network = #testnet;

public shared ({ caller }) func request_deposit_address(orderId : Text) : async Text {
  switch (Trie.find(addressBook, key(orderId), Text.equal)) {
    case (?address) { address };
    case null {
      let derivationPath = [Text.encodeUtf8("lexlink"), Text.encodeUtf8(orderId)];
...
public shared ({ caller }) func confirm_payment(orderId : Text, txid : Text) : async () {
  let address = switch (Trie.find(addressBook, key(orderId), Text.equal)) {
    case (?addr) addr;
    case null throw Error.reject("Unknown order identifier")
  };

  let utxosResponse = await Bitcoin.get_utxos(network, {
    address = address;
    filter = null;
  });

  let matched = Iter.fold(utxosResponse.utxos.vals(), false, func(acc, utxo) {
    acc or Text.equal(bytesToHex(utxo.outpoint.txid), Text.toLowercase(txid))
  });

  payments := Trie.put(payments, key(orderId), Text.equal, {
    txid = txid;
    confirmed = matched
  }).0
}
...
public query func attestation(orderId : Text) : async Text {
  ...
  let confirmed = switch (payment) {
    case (?record) if (record.confirmed) "true" else "false";
    case null "false"
  };
