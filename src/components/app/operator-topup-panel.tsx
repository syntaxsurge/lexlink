  ...
  return (
    <Card className='border-border/60 bg-card/70'>
      <CardHeader>
        ...
        <CardDescription>
          Use the public ckTESTBTC faucet and send funds directly to your Internet Identity.
        </CardDescription>
      </CardHeader>
      <CardContent className='space-y-4 text-sm'>
        <div className='space-y-2 rounded-md border border-border/60 bg-muted/10 p-3'>
          <p className='text-xs uppercase text-muted-foreground'>Operator principal</p>
          <p className='break-all font-mono text-xs'>{snapshot.operator.principal}</p>
        </div>
        <div className='space-y-3 rounded-md border border-primary/40 bg-primary/5 p-3 text-xs text-muted-foreground'>
          ...
          <ol className='list-decimal space-y-2 pl-4'>
            ...
            <li>Refresh the dashboard to see balances update from the ledger.</li>
          </ol>
          <Button
            asChild
            variant='secondary'
        ...
        <p className='text-xs text-muted-foreground'>
          No more deposit allocation or minter polling is required—balances update as soon as the ledger records your faucet transfer.
        </p>
      </CardContent>
    </Card>
  )
